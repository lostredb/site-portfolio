	...
	return (
		<DialogPortal data-slot="dialog-portal">
			...
			<DialogPrimitive.Content
				...
				className={cn(
					"bg-background data-[state=open]:animate-in border-2 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-6 rounded-lg border p-4 shadow-lg duration-200 sm:max-w-lg",
					className,
				)}
				{...props}
			>
...
function DialogInnerContent({
	...
	return (
		<div className={cn("max-h-[75vh] overflow-y-scroll", className)} {...props}>
			{children}
		</div>
	);
}
